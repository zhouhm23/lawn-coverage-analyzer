...
def create_mask_from_color(frame: np.ndarray,
                           hsv_lower: Tuple[int, int, int],
                           hsv_upper: Tuple[int, int, int]) -> np.ndarray:
    """从摄像头帧中按 HSV 颜色范围提取地板蒙版。

    用法: 绿色地板 → hsv_lower=(35, 50, 50), hsv_upper=(85, 255, 255)
    ...
    Returns:
        mask: (H, W) uint8 二值图，255=地板(可通行)，0=障碍物
    """
...
def create_all_passable_mask(frame_shape: Tuple[int, int, int]) -> np.ndarray:
    """创建全白蒙版（整个区域可通行）。

    Args:
        frame_shape: (H, W, 3) 帧尺寸
    Returns:
        mask: (H, W) uint8 全 255
    """
    return np.full((frame_shape[0], frame_shape[1]), 255, dtype=np.uint8)
    ...
    def _calibrate_homography(self, frame: np.ndarray,
        ...
        while scanned < max_scan:
            ...
            if corners is not None:
                ...
                # 丢失一帧，清空重新累积（必须连续稳定，与实时模式一致）
                for cid in cfg.corner_ids:
                    frame_positions[cid].clear()
